...
const SearchAnime = () => {
  ...
  const searchAnime = async (query: string): Promise<void> => {
  ...
  const handleSynopsisClick = (synopsis: string, title: string): void => {
    const cleanedSynopsis = synopsis
      .replace(/\\"/g, '"')
      .replace(/\\n/g, "<br>");
  ...
  const renderSearchResults = () => {
    return searchResults.map((result, index) => {
      const synopsisString = JSON.stringify(result.synopsis)
        .replace(/\\"/g, '"')
        .replace(/'/g, "&apos;")
        .replace(/"/g, "&quot;");
      return (
        <div className="col-md-3 mt-3 g-2" key={index}>
          <div className="card formbg">
            ...
            <div className="card-body text-center">
              <h6 className="text-title fs-4" style={{ color: "#9500ff" }}>
                N. {result.rank}
              </h6>
              <h4
              ...
              <button
                ...
                onClick={() =>
                  handleSynopsisClick(synopsisString, result.title)
                }
              >
                Synopsis
              </button>
        ...
        <div className="modal-dialog modal-dialog-centered modal-xl">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="synopsisModalLabel">
                {selectedTitle}
              </h5>
              <button
                type="button"
